...
public class Product_Listing extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPrefs = AppPreferences.getAppPreferences(Product_Listing.this);
        ...
        ((GridView) findViewById(R.id.main_grid)).setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (array != null) {
                    startActivity(new Intent(Product_Listing.this, brandActivity.class)
    ...
    private void load_view() {
        if (Utility.isNetworkConnected(Product_Listing.this)) {
            Load_Brand lp = new Load_Brand();
            lp.execute();
        } else {
            Toast.makeText(Product_Listing.this, "", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void cart_batch() {
        ...
        DBHandler db = new DBHandler(Product_Listing.this);
    ...
    class Load_Brand extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            MyProgressDialog.show(Product_Listing.this);
        }
        ...
        protected String doInBackground(String... params) {
            String res = null;
            try {
                res = Utility.GetRequest(Product_Listing.this, App_Constant.Category);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return res;
        ...
        protected void onPostExecute(String res) {
            MyProgressDialog.close(Product_Listing.this);
            try {
                if (new JSONObject(res).getString("Msg").equalsIgnoreCase("Success")) {
                    ...
                    for (int i = 0; i < new JSONObject(res).getJSONArray("SuccesModels").length(); i++) {
                        ...
                        map.put("id", mock_object.getString("CategoryId"));
                        map.put("name", mock_object.getString("CategoryName"));
                        map.put("image", mock_object.getString("ImageName"));
                        array.add(map);

                    }
                    // Log.e("dsa ",""+product_array.size());
                    //.setAdapter(new ProductList_Adapter(Product_Listing.this, array));
                    adapter = new ProductList_Adapter(Product_Listing.this, array);
        ...
        switch (v.getId()) {
            ...
            case R.id.ic_cart:
                ...
                if(U_id.equalsIgnoreCase("")){
                    Toast.makeText(Product_Listing.this,"You are not Login User",Toast.LENGTH_SHORT).show();
                }else {
                    List<MyCart_property> shops = db.getAllProduct(U_id);
                    if (shops.size() > 0) {
                        startActivity(new Intent(Product_Listing.this, My_Cart.class));
                    } else {
                        Toast.makeText(Product_Listing.this, "Your cart is empty", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
            case R.id.ic_search:
